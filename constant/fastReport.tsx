...
export const fetchDayDataTransactions = async (day: any) => {
    console.log("day : ", day)
    const query = `
  SELECT 
        ...
        WHERE  DATE(DATE_ADD(t.created_at, INTERVAL 2 HOUR)) = '${day}'  -- فلترة حسب اليوم المحدد
...
export const fetchDayDataPermits = async (day: any) => {
    console.log("day : ", day)
    const query = `
        SELECT 
...
WHERE  DATE(DATE_ADD(ft.transaction_date, INTERVAL 2 HOUR)) = '${day}'
...
export const fetchDayDataBanks = async (day: any) => {
    console.log("day : ", day)
    const query = `
        SELECT 
...
WHERE DATE(DATE_ADD(bt.created_at, INTERVAL 2 HOUR)) = '${day}'
...
export const fetchDayDataBridgePoint = async (day: any) => {
    console.log("day : ", day)
    const query = `
         SELECT 
    i.id AS invoke_id,
...
WHERE DATE(DATE_ADD(FROM_UNIXTIME(i.date), INTERVAL 2 HOUR)) = '${day}'
